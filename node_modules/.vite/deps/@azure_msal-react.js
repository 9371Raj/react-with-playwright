...
var MsalContext = K(defaultMsalContext);
...
function MsalProvider({ instance, children }) {
  ...
  const [state, updateState] = h(reducer, void 0, () => {
    return {
  ...
  return Rn.createElement(MsalContext.Provider, { value: contextValue }, children);
}

// node_modules/@azure/msal-react/dist/hooks/useMsal.js
var useMsal = () => x(MsalContext);
...
function AuthenticatedTemplate({ username, homeAccountId, localAccountId, children }) {
  ...
  if (isAuthenticated2 && context.inProgress !== InteractionStatus.Startup) {
    return Rn.createElement(Rn.Fragment, null, getChildrenOrFunction(children, context));
  }
  return null;
}
...
function useMsalAuthentication(interactionType, authenticationRequest, accountIdentifiers) {
  ...
  const [[result, error], setResponse] = d([null, null]);
  ...
  const interactionInProgress = A(inProgress !== InteractionStatus.None);
  y(() => {
    interactionInProgress.current = inProgress !== InteractionStatus.None;
  }, [inProgress]);
  const shouldAcquireToken = A(true);
  y(() => {
    if (!!error) {
  ...
  const login = q(async (callbackInteractionType, callbackRequest) => {
  ...
  const acquireToken = q(async (callbackInteractionType, callbackRequest) => {
...
function MsalAuthenticationTemplate({ interactionType, username, homeAccountId, localAccountId, authenticationRequest, loadingComponent: LoadingComponent, errorComponent: ErrorComponent, children }) {
  ...
  if (msalAuthResult.error && context.inProgress === InteractionStatus.None) {
    if (!!ErrorComponent) {
      return Rn.createElement(ErrorComponent, { ...msalAuthResult });
    }
    throw msalAuthResult.error;
  }
  if (isAuthenticated2) {
    return Rn.createElement(Rn.Fragment, null, getChildrenOrFunction(children, msalAuthResult));
  }
  if (!!LoadingComponent && context.inProgress !== InteractionStatus.None) {
    return Rn.createElement(LoadingComponent, { ...context });
  }
  return null;
}
...
var withMsal = (Component) => {
  const ComponentWithMsal = (props) => {
    const msal = useMsal();
    return Rn.createElement(Component, { ...props, msalContext: msal });
  };
